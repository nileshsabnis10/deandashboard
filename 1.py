# Dean Dashboard — UI v4.1.3 (Read-Only, main-screen filters, bugfix for missing Value column)
# Built: 19 Sep 2025, 10:22 AM IST
# Notes:
#   • Bugfix: Added check for 'Value' column in _AppSettings to prevent KeyError.
#   • Added download button for locked components list in Class View.
...
# Constants / Meta
# ==============================
DASHBOARD_VERSION = "4.1.3"
LAST_BUILD_STR = "19 Sep 2025, 10:22 AM IST"
...
def get_class_cutoff_display(ssid: str) -> str:
    ...
    # Check if both required columns exist to prevent KeyError
    if appset.empty or "Key" not in appset.columns or "Value" not in appset.columns:
        return ""
...
with title_col:
    st.title("📊 Dean's Progress Dashboard")
...
if nav == "Overview":
    ...
    if ssid:
        ...
        if tbl.empty:
            ...
            locked_df = tbl[tbl["Status"] == "🔒 Locked"].copy()
            if not locked_df.empty:
                report_df = locked_df[["CourseCode", "Course", "Component"]]
                locked_csv_bytes = df_to_csv_bytes(report_df)
                locked_fname = _slug(dept, klass, "Locked_Components") + ".csv"
                st.download_button(
                    label="⬇️ Download Locked Components (CSV)",
                    data=locked_csv_bytes,
                    file_name=locked_fname,
                    mime="text/csv",
                    key=f"c_locked_{ssid}_{klass}"
                )
